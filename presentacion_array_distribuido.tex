...
\begin{tikzpicture}[scale=0.9, transform shape]
    ...
    % Worker nodes
    \visible<2->{
    \node[worker] (w1) at (-3,0) {Worker 0};
    \node[worker] (w2) at (0,0) {Worker 1};
    \node[worker] (w3) at (3,0) {Worker 2};
    }
    
    % Client
    \visible<3->{
    \node[client] (client) at (0,6) {Cliente};
    }
    
    % Connections
    \visible<2->{
    \draw[socket] (master) -- (w1) node[midway, left] {TCP};
    \draw[socket] (master) -- (w2) node[midway, right] {TCP};
    \draw[socket] (master) -- (w3) node[midway, right] {TCP};
    }
    
    \visible<3->{
    \draw[arrow, blue, thick] (client) -- (master);
    }
    
    % Data distribution
    \visible<4->{
    \node[data, below=0.3cm of w1] (d1) {Segmento 0};
    \node[data, below=0.3cm of w2] (d2) {Segmento 1};
    \node[data, below=0.3cm of w3] (d3) {Segmento 2};
    }
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.8]
    % Original array
    \visible<1->{
    \draw[thick] (0,4) rectangle (8,5);
    \node at (4,4.5) {Array Original (10,000 elementos)};
    }
    
    % Segmentation arrows
    \visible<2->{
    \draw[arrow, red] (2,3.8) -- (2,3);
    \draw[arrow, red] (4,3.8) -- (4,3);
    \draw[arrow, red] (6,3.8) -- (6,3);
    }
    
    % Segments
    \visible<3->{
    \draw[fill=green!20] (0,2) rectangle (2.5,3);
    \node at (1.25,2.5) {Seg 0};
    \node at (1.25,1.7) {\small 3,333 elem};
    
    \draw[fill=blue!20] (3,2) rectangle (5.5,3);
    \node at (4.25,2.5) {Seg 1};
    \node at (4.25,1.7) {\small 3,333 elem};
    
    \draw[fill=orange!20] (6,2) rectangle (8.5,3);
    \node at (7.25,2.5) {Seg 2};
    \node at (7.25,1.7) {\small 3,334 elem};
    }
    
    % Workers
    \visible<4->{
    \draw[arrow] (1.25,1.5) -- (1.25,0.5);
    \draw[arrow] (4.25,1.5) -- (4.25,0.5);
    \draw[arrow] (7.25,1.5) -- (7.25,0.5);
    
    \node[worker, scale=0.7] at (1.25,0) {Worker 0};
    \node[worker, scale=0.7] at (4.25,0) {Worker 1};
    \node[worker, scale=0.7] at (7.25,0) {Worker 2};
    }
\end{tikzpicture}
\end{center}
...
\begin{block}{Algoritmo de Segmentación}
    \begin{itemize}
        \item División equitativa: $\text{tamaño\_segmento} = \text{total} / \text{workers}$
...
\begin{tikzpicture}[scale=0.8]
    % Worker node
    \node[worker, minimum width=4cm] (worker) at (0,4) {Worker Node};
    
    % CPU cores
    \visible<2->{
    \node[draw, circle, fill=yellow!30] (c1) at (-1.5,2) {Core 0};
    \node[draw, circle, fill=yellow!30] (c2) at (-0.5,2) {Core 1};
    \node[draw, circle, fill=yellow!30] (c3) at (0.5,2) {Core 2};
    \node[draw, circle, fill=yellow!30] (c4) at (1.5,2) {Core 3};
    ...
    % Thread pool
    \visible<3->{
    \node[draw, rectangle, fill=blue!10, minimum width=4cm] (pool) at (0,0.5) {ThreadPool};
    \draw[arrow] (c1) -- (pool);
    \draw[arrow] (c2) -- (pool);
    \draw[arrow] (c3) -- (pool);
    \draw[arrow] (c4) -- (pool);
    ...
    % Data chunks
    \visible<4->{
    \node[data, scale=0.7] at (-3,-1) {Chunk 0};
    \node[data, scale=0.7] at (-1,-1) {Chunk 1};
    \node[data, scale=0.7] at (1,-1) {Chunk 2};
    \node[data, scale=0.7] at (3,-1) {Chunk 3};
    }
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.8]
    ...
    % Master
    \node[master, scale=0.7] at (0,2) {Master};
    
    % Worker healthy
    \visible<1-3>{
    \node[worker, scale=0.7] at (2,2) {Worker};
    \draw[<->, green, thick] (0.8,2) -- (1.2,2);
    }
    
    % Heartbeats
    \visible<2->{
    \foreach \x in {2,3,4} {
        \draw[arrow, blue] (\x,1.5) -- (\x,2.5);
        \node[scale=0.6] at (\x,1.3) {HB};
    }
    }
    
    % Worker fails
    \visible<4->{
    \node[worker, scale=0.7, fill=red!30] at (5,2) {Worker};
    \draw[->, red, thick] (4.5,2) -- (5.5,2);
    \node[scale=0.6] at (5,1.3) {Timeout};
    }
    
    % Detection
    \visible<5->{
    \node[draw, rectangle, fill=yellow!20] at (6,3) {Detección};
    \draw[arrow, red] (5,2.3) -- (6,2.7);
    }
    
    % Recovery
    \visible<6->{
    \node[draw, rectangle, fill=green!20] at (7.5,3) {Recuperación};
    \draw[arrow] (6.5,3) -- (7,3);
    }
\end{tikzpicture}
\end{center}
...
\begin{block}{Mecanismo de Heartbeat}
    \begin{itemize}
        \item Intervalo: 3 segundos
        \item Timeout: 10 segundos
        \item Acción: Marcar nodo como caído
    \end{itemize}
\end{block}
}
\end{frame}
...
\begin{block}{Logros}
    \begin{itemize}
        \item<1-> Librería funcional en Java y Python
...
\normalsize
GitHub: \url{github.com/andre/distributed-array-lib}
